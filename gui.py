...
def run_processing(input_file):
    try:
        ...
        output_json = 'output_TSC.json'
        output_docx = os.path.join(downloads_folder, 'Updated_CP_Document2.docx')
        
        # Paths to your template files
        word_template = 'CP Template_jinja.docx'  # Adjust if necessary
        ...
        subprocess.run([
            ...
            word_template,          # Word template for json_docu_replace.py
            output_docx             # Final output DOCX file
        ], check=True)
        
        status_label.config(text="Processing complete.")
        messagebox.showinfo("Success", f"File processed and saved to {downloads_folder}")
