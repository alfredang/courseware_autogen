...
async def generate_assessment_plan(context, name_of_organisation, model_client, sfw_dataset_dir=None):
    """
    ...
    if sfw_dataset_dir is None:
        ...
        def generate_asr_document(context: dict) -> str:
        ...
        if not is_evidence_extracted(context):
            ...
            evidence = await extract_assessment_evidence(structured_data=context, model_client=evidence_model_client)
    ...
    team = RoundRobinGroupChat([excel_data_retriever, ap_assistant], termination_condition=text_termination)
    ...
    asr_doc_path = generate_asr_document(context=context)

    if ap_doc_path is None:
        raise Exception("Error: Failed to generate Assessment Plan!")
    elif asr_doc_path is None:
        raise Exception("Error: Failed to generate Assessment Summary!")
    else:
        return ap_doc_path, asr_doc_path
