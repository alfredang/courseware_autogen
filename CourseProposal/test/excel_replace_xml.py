...
def process_excel_update(json_data_path, excel_template_path, output_excel_path, ensemble_output_path):
    ...
    cell_replacement_map = {
        ...
        #"#Combined_LO": {"sheet": "3 - Instructional Design", "cell": "B7", "json_key": "#Combined_LO"}
    }
    ...
    try:
        ...
        if "3 - Summary" in sheet_mapping:
            ...
            # Create instructional and assessment dataframes (we know these are not empty based on previous code)
            instructional_2_df = create_instructional_dataframe(ensemble_output) if 'instructional_2_df' not in locals() else instructional_2_df
            methods_df = create_assessment_dataframe(ensemble_output) if 'methods_df' not in locals() else methods_df
            
            # For course_df (instructional_df in this context), we need to ensure it contains data
            # If instructional_df exists but is empty, let's create a synthetic one from TSC data
            synthetic_course_df = None
            if 'instructional_df' in locals() and instructional_df.empty:
                print("Instructional course dataframe is empty. Creating a synthetic one for summary...")
                # Create a synthetic course dataframe from the Learning Units in the ensemble output
                synthetic_course_df = pd.DataFrame()
                ensemble_data = load_json_file(ensemble_output_path)
                ...
                synthetic_course_df["LU#"] = [f"LU{i+1}" for i in range(len(learning_units))]
                synthetic_course_df["Learning Unit Title"] = [lu.split(":", 1)[1].split("(", 1)[0].strip() if ":" in lu else lu for lu in learning_units]
                synthetic_course_df["LO#"] = [f"LO{i+1}" for i in range(len(learning_outcomes))]
                synthetic_course_df["Learning Outcome"] = [lo.split(":", 1)[1].strip() if ":" in lo else lo for lo in learning_outcomes[:len(learning_units)]]
                synthetic_course_df["Topic (T#: Topic title)"] = ["T1: Placeholder topic"] * len(learning_units)
                synthetic_course_df["Applicable K&A Statement"] = ["K1: Sample knowledge statement"] * len(learning_units)
                synthetic_course_df["Mode of Assessment"] = ["Written Exam"] * len(learning_units)
            
            # Use the synthetic course dataframe if instructional_df is empty
            input_course_df = synthetic_course_df if synthetic_course_df is not None else instructional_df
            
            summary_df = create_summary_dataframe(input_course_df, instructional_2_df, methods_df)
