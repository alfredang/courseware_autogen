...
def run_processing(input_file):
    try:
        ...
        output_json = os.path.join(temp_dir, 'output_TSC.json')
        output_docx = os.path.join(temp_dir, 'Updated_CP_Document2.docx')

        # Paths to the template files for root main.py
        word_template = os.path.join(os.path.dirname(__file__), 'CP Template_jinja.docx')

        # Paths to the template files for CV/main.py
        word_template_1 = os.path.join(os.path.dirname(__file__), 'CP_validation_template_bernard.docx')
        word_template_2 = os.path.join(os.path.dirname(__file__), 'CP_validation_template_dwight.docx')
        word_template_3 = os.path.join(os.path.dirname(__file__), 'CP_validation_template_ferris.docx')

        # Ensure all templates exist
        for template in [word_template, word_template_1, word_template_2, word_template_3]:
            if not os.path.exists(template):
                st.error(f"Template file {template} not found.")
                return

        # Step 1: Run root directory main.py
        ...
        if result.returncode != 0:
            st.error(f"An error occurred during processing main.py:\n{result.stderr}")
            return
        ...
        # Step 2: Run CV/main.py with different templates
        cv_result = subprocess.run([
            ...
            word_template_2,
            word_template_3,
            temp_dir  # Pass temp_dir as the output directory
        ], capture_output=True, text=True, cwd=os.path.join(os.path.dirname(__file__), 'CV'))

        if cv_result.returncode != 0:
            st.error(f"An error occurred during processing CV/main.py:\n{cv_result.stderr}")
            return

        st.success("Processing complete for Course Validation forms.")
        # The output files are in temp_dir, with filenames based on the templates
        output_files = [
            os.path.join(temp_dir, f"{os.path.splitext(os.path.basename(word_template_1))[0]}_updated.docx"),
            os.path.join(temp_dir, f"{os.path.splitext(os.path.basename(word_template_2))[0]}_updated.docx"),
            os.path.join(temp_dir, f"{os.path.splitext(os.path.basename(word_template_3))[0]}_updated.docx")
        ]
